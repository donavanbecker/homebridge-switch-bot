...
export class Curtain {
  ...
  setNewTargetTimer!: any | NodeJS.Timeout;
  ...
  handleTargetPositionSet(value, callback) {
    ...
    this.setNewTargetTimer
      ...
      .subscribe(async () => {
        try {
          await this.doCurtainUpdate.next();
        } catch (e) {
          this.platform.log.error(JSON.stringify(e.message));
          this.platform.log.debug('Curtain %s -', this.accessory.displayName, JSON.stringify(e));
          this.apiError(e);
  ...
  public setMinMax() {
    if (this.platform.config.options?.curtain?.set_min) {
      if (this.CurrentPosition - 1 <= this.platform.config.options?.curtain?.set_min) {
        this.CurrentPosition = 0;
      }
    }
    if (this.platform.config.options?.curtain?.set_max) {
      if (this.CurrentPosition + 1 >= this.platform.config.options?.curtain?.set_max) {
